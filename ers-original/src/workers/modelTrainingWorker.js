...
// Normaliza um valor entre 0 e 1, dado um mínimo e máximo
// Se o valor for igual ao mínimo, retorna 0; 
// se for igual ao máximo, retorna 1; caso contrário, retorna um valor proporcional entre 0 e 1
...
function makeContext(products, users) {
    ...
    const productAvgAgeNorm = Object.fromEntries(
        products.map(product => { 
            const avg = ageSums[product.name]
    ...
    return {
        ...
        // idade normalizada + preço normalizado + one-hot de categorias + one-hot de cores
...
const oneHotWeighted = (index, length, weight) => 
 ...
 function encodeProduct (product, context) {
    ...
    // e aplicar o peso na recomendaçao
...
function createTrainingData (context) {
    const inputs = []
    const labels = []
    context.users.filter(u => u.purchases.length)
        .forEach(user => { 
        ...
        context.products.forEach(product => {
            ...
            // Aqui você pode armazenar ou usar os vetores e rótulos para treinar seu modelo
            // Por exemplo, você poderia empilhar os vetores de usuário e produto e associá-los ao rótulo
            //combinar usuário e produto em um único vetor de entrada
